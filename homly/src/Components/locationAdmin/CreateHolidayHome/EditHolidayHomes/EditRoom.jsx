...
const EditRoom = ({
    ...
    axios
      ...
      .then((res) => {
        ...
        for (let i = 0; i < rental.length; i++) {
          ...
          setRentalArray(rental); // Use functional updat
        }
        setOpen(true);
        setEditIndex(index);
        setIsEditMode(true);
  ...
  const handleRoomCodeChange = (e) => {
    const roomCodeExists = roomArray.some(
      (room) => room.roomCode === e.target.value
    );
    if (roomCodeExists) {
      setRoomExist(true);
    } else {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < settingRoomRentalArray.length; i++) {
      if (
        ...
        setValues((prevValues) => ({
          ...prevValues,
          roomRental: settingRoomRentalArray[i].rental,
        }));
        valid = true;
        break;
      }
            ...
            <DialogContent sx={{ maxHeight: "350px" }}>
              ...
              >
                ...
                <TextField
                  ...
                  helperText={roomExist ? "Already exist" : ""}
