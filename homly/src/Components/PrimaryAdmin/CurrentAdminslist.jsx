    ...
    return(
        <Box
              ...
              >
                  ...
                  setOpen={setOpen}
                />
               {load===true?<h1>Loading...</h1>:<Box>
               {blacklistedusers.filter(item=>item.Disabled===false).map((item) => {
                  return (
                    <CurrentAdminCard data={item} key={item.AdminNo} fetchadmins={fetchadmins} setsnacktext={setsnacktext} handlesnack={handleClick} loading={load}/>
                  );
                })}
               </Box> }
              </Box> 
